...
26 elements, 38 components, and 13 blocks:
...
| component | `accordion` `alert` `alert-dialog` `breadcrumb` `button-group` `calendar` `card` `carousel` `chart` `collapsible` `combobox` `command` `context-menu` `data-table` `date-picker` `dialog` `drawer` `dropdown-menu` `empty-state` `field` `footer` `hover-card` `input-group` `list` `menubar` `navbar` `navigation-menu` `pagination` `popover` `select` `sheet` `sidebar` `stat` `table` `tabs` `toast` `toolbar` `tooltip` |
